!----------------------------------------------------------------------------------------
!     READS the transition dipole matrix elements and calculates the k-resolved
!     Dielectric tensor
!     
...
!-----------------------------------------------------------------------------------------
      ...
      COMPLEX*8, allocatable :: O_E (:,:,:)
      real :: erange_max, erange_min
...
!---------------------------------------------!
      ...
      allocate (eps_k(3,3,nk,n_L))
      allocate (O_E(nk,3,3))
      O_E (:,:,:) = CMPLX(0.0d0,0.0d0)
	eps_k (:,:,:,:)= CMPLX(0.d0,0.d0)
      !
        ...
        do ik = 1, nk 
         ...
         do iv = 1, nbnd_max
          ...
          do ic = iv+1, nbnd_max
           ...
           !
           fac = (focc(ik,iv)-focc(ik,ic)) * (2*et) / & 
!           (et**2 - e_L**2 - (0,1)*2*delta*e_L) &
           ...
           * ry_to_eV
           ...
           do ipol = 1, 3
            ...
            do jpol = 1, 3
             ...
             eps_k (ipol,jpol,ik,iL) = eps_k (ipol,jpol,ik,iL)+&
             (fac*TDMat(ik,ic,iv,ipol)*CONJG(TDMat(ik,ic,iv,jpol)))
         ...
         enddo   ! ic
         !
         !if ( e_L < erange_max .AND. e_L >= erange_min) then
         !  O_E (ik,:,:) = O_E (ik,:,:) + eps_k (:,:,iL)
...
!      close (21) 
      ...
      !
      open (21,file='EPS_vs_k.dat',action='WRITE')
      do ik = 1, nk
       write (21,1001) ik, & 
         O_E(ik,1,1), O_E(ik,1,2), O_E(ik,1,3), &
         O_E(ik,2,1), O_E(ik,2,2), O_E(ik,2,3), &
         O_E(ik,3,1), O_E(ik,3,2), O_E(ik,3,3)
      enddo
      close (21)
      !
      1001 format( I5, 1X, 9(e16.8,1X,e16.8))
      !
